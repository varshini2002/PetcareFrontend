...
function BookingAppointment() {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            axios.get(`http://65.0.4.44:8090/api/v1/auth/getuser?email=${email}`)
                .then(response => {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            const fetchPets = async () => {
                try {
                    const response = await axios.get(`http://65.0.4.44:8090/getPetByOwnerEmail?email=${email}`);
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await axios.post('http://65.0.4.44:8090/createBooking', {
