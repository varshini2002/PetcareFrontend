...
function ViewAppointments() {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            const fetchAppointments = async () => {
                try {
                    const response = await axios.get(`http://172.31.40.114:8090/getBookingByOwnerEmail?ownerEmail=${email}`);
    ...
    const handleCancelAppointment = async (petName) => {
        ...
        try {
            ...
            await axios.delete(`http://172.31.40.114:8090/deleteBooking`, {
                params: {
